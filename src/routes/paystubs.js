...
// --- PDF GENERATOR ---
// ✅ supports inline vs attachment using res.locals.disposition
async function generateSinglePagePdf(paystub, res) {
  ...
  // ✅ default is inline (preview). certified route sets attachment.
  const disposition = res.locals.disposition || 'inline';
  ...
  function drawStub(startY) {
    ...
    doc.text('Check Date:', dateX, dateY);
    doc.text(checkDate, 500, dateY, { align: 'right' });
    doc.text('Pay Period Beginning:', dateX, dateY + 12);
    doc.text(pStart, 500, dateY + 12, { align: 'right' });
    doc.text('Pay Period Ending:', dateX, dateY + 24);
    doc.text(pEnd, 500, dateY + 24, { align: 'right' });
...
// ✅ Certified copy / forced download
router.get('/:id/certified', async (req, res) => {
  try {
    ...
    // force download
    res.locals.disposition = 'attachment';
    await generateSinglePagePdf(paystub, res);
  } catch (err) {
    res.status(500).send(`PDF Error: ${err.message}`);
  }
