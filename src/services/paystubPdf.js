...
async function generateAdpPaystubPdf(paystub) {
  try {
    ...
    try {
      const imagePath = path.join(__dirname, 'nwf-background.png');
      if (fs.existsSync(imagePath)) {
        const imageBuffer = fs.readFileSync(imagePath);
        backgroundImageBase64 = imageBuffer.toString('base64');
      }
    } catch (error) {
      console.warn('Could not load local background image, using fallback URL');
    }

    const backgroundUrl = backgroundImageBase64 
      ? `data:image/png;base64,${backgroundImageBase64}`
      : "https://www.nwfpayroll.com/nwf-paystub-bg.png"; // Fallback URL

    // -------------------------------
    // TEMPLATE HTML WITH BACKGROUND ON EVERY PAGE
    // -------------------------------
...
<body>
  ...
  <!-- Second Page (if needed for additional content) -->
  <div class="page">
    <img src="<%= backgroundUrl %>" class="page-bg" />
    <div class="page-content">
      <!-- Additional content can go here -->
      <div style="text-align: center; margin-top: 50%;">
        <h3>NWF PAYROLL SERVICES</h3>
        <p>PAYROLL FOR SMALL BUSINESSES & SELF-EMPLOYED</p>
      </div>
    </div>
  </div>
</body>
